...
class MatiereController extends AbstractController
{
    ...
    public function new(Request $request, EntityManagerInterface $entityManager): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            return $this->redirectToRoute('app_emplois', [], Response::HTTP_SEE_OTHER);
        }
    ...
    #[Route('/clear/{id}',name: 'app_matiere_clear',methods: ['POST'])]
    public function clear(Request $request,Professeur $professeur,MatiereRepository $matiereRepository,EntityManagerInterface $entityManager ):Response
    {
        if ($this->isCsrfTokenValid('delete_all_matieres'.$professeur->getId(), $request->request->get('_Cleartoken'))) {
            // find all matiere by professor id
            $matieres=$matiereRepository->findBy(['Professeur'=>$professeur]);
